...
- **Measured CPU latencies (fp32, 0.6B)**: embed ≈ 0.8s short / 2.5s @512 tokens; rerank ≈ 1.1s short / 3s @512. Hybrid retrieval is budgeted: 5 rerank candidates × 256 tokens ≈ 10s, run on the retrieval worker; send_chat defers up to 20s then falls back to BM25 (µs). The reranker loads lazily on first rerank (saves ~2.3GB RAM until then).
